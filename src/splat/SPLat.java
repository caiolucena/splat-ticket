		...
		try {
			...
			for (Class<?> clazz : clazzes) {
				...
				for (Method mTest : methods) {
					Annotation[] annotations = mTest.getAnnotations();
					
					for (int i = 0; i < annotations.length; i++) {
						if(annotations[i].toString().contains("Test")) {
					...
					if (annotation != null) {
						...
						do {
							...
							// execute or not
							if (sample) {
								Result r = runTest(true, clazz, mTest);
...
//							System.err.println(traces.size());
					...
					//aqui ante do pTestResultsrox teste. API difff		
					}
			}
		} catch (Exception e) {
